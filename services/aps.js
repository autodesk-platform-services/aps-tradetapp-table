...
service.getFinancials = async (token) => {
    ...
    const response = await fetch("https://developer.api.autodesk.com/construction/tradetapp/v2/financials", requestOptions);
...
service.getQualifications = async (token) => {
    const myHeaders = new Headers();
    myHeaders.append("Accept", "application/json");
    myHeaders.append("Authorization", "Bearer " + token.access_token);
    ...
    const response = await fetch("https://developer.api.autodesk.com/construction/tradetapp/v2/qualifications", requestOptions);
    const result = await response.json();
